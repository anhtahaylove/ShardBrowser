...
- **Internal updater-signed build:** run the Release workflow manually and leave
  ...
  custom workflow, pass the matching tag such as `v0.1.26`, and download the
...
- **Public release:** push a version tag such as `v0.1.26`, or manually run the
...
```powershell
gh workflow run Release --repo anhtahaylove/ShardBrowser --ref codex/v0.1.26-startup-security -f tag=v0.1.26 -f publish_release=false
```
