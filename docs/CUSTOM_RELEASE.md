...
- **Internal updater-signed build:** run the Release workflow manually and leave
  ...
  custom workflow, pass the matching tag such as `v0.1.25`, and download the
...
- **Public release:** push a version tag such as `v0.1.25`, or manually run the
...
```powershell
gh workflow run Release --repo anhtahaylove/ShardBrowser --ref codex/v0.1.25-mcp-archive-fix -f tag=v0.1.25 -f publish_release=false
```
