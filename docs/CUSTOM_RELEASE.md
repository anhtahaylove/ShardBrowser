...
- **Internal updater-signed build:** run the Release workflow manually and leave
  ...
  custom workflow, pass the matching tag such as `v0.1.24`, and download the
...
- **Public release:** push a version tag such as `v0.1.24`, or manually run the
...
```powershell
gh workflow run Release --repo anhtahaylove/ShardBrowser --ref codex/v0.1.24-autostart -f tag=v0.1.24 -f publish_release=false
```
