...
```text
https://github.com/anhtahaylove/ShardBrowser/releases/download/v0.1.13/ShardX-MCP.tar.gz
```
...
- **Internal signed build:** run the Release workflow manually and leave
  ...
  custom workflow, pass the matching tag such as `v0.1.13`, and download the
...
- **Public release:** push a version tag such as `v0.1.13`, or manually run the
...
```powershell
gh workflow run Release --repo anhtahaylove/ShardBrowser --ref codex/custom-integration-v0.1.13 -f tag=v0.1.13 -f publish_release=false
```
