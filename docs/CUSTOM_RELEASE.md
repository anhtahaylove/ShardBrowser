...
- **Internal updater-signed build:** run the Release workflow manually and leave
  ...
  custom workflow, pass the matching tag such as `v0.1.22`, and download the
...
- **Public release:** push a version tag such as `v0.1.22`, or manually run the
...
```powershell
gh workflow run Release --repo anhtahaylove/ShardBrowser --ref codex/custom-integration-v0.1.12 -f tag=v0.1.22 -f publish_release=false
```
