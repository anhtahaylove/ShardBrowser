...
test("updater requires consent, reports progress, and requests restart", async ({ page }) => {
  ...
  await expect(page.getByText("Update available → v0.1.22")).toBeVisible();
  const trigger = page.getByRole("button", { name: /ShardX Launcher v0.1.21/ });
  await trigger.focus();
  await trigger.click();
...
test("updater surfaces invalid signatures inline", async ({ page }) => {
  ...
  await page.getByRole("button", { name: /ShardX Launcher v0.1.21/ }).click();
  await page.getByRole("button", { name: "Download update" }).click();
